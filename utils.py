...
def prepare_data(df, input_window_len=32, pred_steps=1, overlaps=0, train_test_split=None, tracks_len_list=None, max_instruments=None):
    '''
        ...
        if sum(note) == 0:
            ...
            for inst in note:
                ...
                if inst == 0:
                    ...
                    drum_track.append(Message('note_on', channel=9, note=inst, velocity=80, time=notes_from_last_message*ticks_per_32note))
                    same_note_count += 1
                else:
                    drum_track.append(Message('note_on', channel=9, note=inst, velocity=80, time=0))
                    same_note_count += 1
    return new_song


def concat_all_midi_to_df(file_list, return_tracks_len_list=True):
    # loop through all the midis in provided root_dir and create df
    df_lists = []
    for file_name in file_list:
        midiprocessor = MidiProcessor(MIDI_DIR + '/' + file_name)
